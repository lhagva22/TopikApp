...
import { examAuthUseCases, examUseCases } from '../dependencies';
import type { SubmitLevelTestResponse } from '../../domain/types';
import type { Question } from './types';

const { width: screenWidth } = Dimensions.get('window');
type ExamInterfaceRoute = RouteProp<RootDrawerParamList, 'ExamInterface'>;
type ExamInterfaceNavigation = DrawerNavigationProp<RootDrawerParamList, 'ExamInterface'>;
...
export const ExamInterface = () => {
  ...
  const initialQuestions = useMemo<Question[]>(() => params.questions ?? [], [params.questions]);
  ...
  const syncProfile = useCallback(async () => {
    const response = await examAuthUseCases.getProfile();
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const result = isLevelTest
        ? await examUseCases.submitLevelTest(currentSessionId, answerList, timeSpent)
        : await examUseCases.submitExam(currentSessionId, answerList, timeSpent);
      ...
      if (result.success && 'result' in result) {
        ...
        const nextLevelTest =
          isLevelTest && 'nextLevelTest' in result
            ? (result as Extract<SubmitLevelTestResponse, { success: true }>).nextLevelTest ?? null
            : undefined;
  ...
  if (loading) {
    return (
      <StateView variant="loading" title="Шалгалт бэлтгэж байна..." style={styles.centerContainer} />
    );
  }
  ...
  if (error) {
    return (
      <StateView
        ...
        onAction={() => startExam().catch(() => undefined)}
  ...
  if (questions.length === 0) {
    return (
      <StateView variant="loading" title="Асуултуудыг ачаалж байна..." style={styles.centerContainer} />
    );
  }
