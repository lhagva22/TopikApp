...
import { authUseCases } from '../dependencies';
...
const ForgotPassword = () => {
  ...
  const sendOtp = async () => {
    ...
    try {
      const response = await authUseCases.forgotPassword({ email: email.trim() });
  ...
  const resetPassword = async () => {
    ...
    if (password.length < 8) {
      setError('Нууц үг 8 ба түүнээс дээш тэмдэгттэй байх ёстой.');
      return;
    }
    ...
    try {
      const response = await authUseCases.resetPassword({
        resetToken,
        password,
      });
  ...
  const verifyOtp = async () => {
    ...
    try {
      const response = await authUseCases.verifyResetOtp({
        email: email.trim(),
        token: otp.trim(),
      });
