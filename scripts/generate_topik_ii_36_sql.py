...
PAPERS_PDF = DOWNLOADS / "36th TOPIK II Papers - Listening + Writing + Reading.pdf"
LISTENING_PDF = DOWNLOADS / "36th TOPIK II - Listening Text.pdf"
ANSWER_PDF = DOWNLOADS / "36th TOPIK II Answers.pdf"
LISTENING_AUDIO = DOWNLOADS / "36th-TOPIK-II-Listening-Audio-File-64k-mono.mp3"

OUTPUT_SQL = ROOT / "backend" / "sql" / "seeds" / "mock_tests" / "topik_ii" / "topik_ii_36.sql"
ASSETS_ROOT = ROOT / "topik_ii_36_assets"
UPLOAD_ROOT = ASSETS_ROOT / "upload" / "topik-ii-36"
BACKEND_MEDIA_ROOT = ROOT / "backend" / "public" / "topik-ii-36"

STORAGE_BASE_URL = "https://ywtxdfwntobzegrlyplw.supabase.co/storage/v1/object/public/mock%20test%20files"
AUDIO_URL = f"{STORAGE_BASE_URL}/topik-ii-36/audio/36th-TOPIK-II-Listening-Audio-File-64k-mono.mp3"
MOCK_TEST_ID_SQL = (
    "(SELECT id FROM mock_test_bank "
    "WHERE exam_type = 'TOPIK_II' AND test_number = 36 "
...
READING_PAGE_RANGES = range(19, 41)

LISTENING_IMAGE_URLS: dict[int, str] = {}

LISTENING_OPTION_IMAGE_URLS = {
    1: [f"{STORAGE_BASE_URL}/topik-ii-36/listening/q1_option_{index}.png" for index in range(1, 5)],
    2: [f"{STORAGE_BASE_URL}/topik-ii-36/listening/q2_option_{index}.png" for index in range(1, 5)],
    3: [f"{STORAGE_BASE_URL}/topik-ii-36/listening/q3_option_{index}.png" for index in range(1, 5)],
}

READING_IMAGE_URLS = {
    55: f"{STORAGE_BASE_URL}/topik-ii-36/reading/q55.png",
    56: f"{STORAGE_BASE_URL}/topik-ii-36/reading/q56.png",
    57: f"{STORAGE_BASE_URL}/topik-ii-36/reading/q57.png",
    58: f"{STORAGE_BASE_URL}/topik-ii-36/reading/q58.png",
    59: f"{STORAGE_BASE_URL}/topik-ii-36/reading/q59.png",
    60: f"{STORAGE_BASE_URL}/topik-ii-36/reading/q60.png",
}
...
def clean_listening_page(text: str) -> str:
    ...
    text = text.replace("듣기 통합(1번～50번)", "")
    text = re.sub(r"제36회 한국어능력시험II\s*B형\(듣기 통합\)", "", text)
    text = re.sub(r"제36회 한국어능력시험II\s*B형\s*\(듣기 통합\)", "", text)
...
def clean_reading_page(text: str) -> str:
    text = re.sub(r"제36회 한국어능력시험II\s*B형\s*2교시\(읽기\)", "", text)
...
def build_sql(questions: list[dict[str, object]]) -> str:
    lines = [
        ...
        ");",
        "",
        f"UPDATE mock_test_bank SET title = 'TOPIK II 36-р шалгалт', description = '36-р албан ёсны TOPIK II шалгалт', total_questions = 100, duration = 130, listening_questions = 50, reading_questions = 50, updated_at = NOW() WHERE id = {MOCK_TEST_ID_SQL};",
        "",
