...
PAPERS_PDF = DOWNLOADS / "52nd TOPIK II Papers.pdf"
LISTENING_PDF = DOWNLOADS / "52nd TOPIK II Listening Text.pdf"
ANSWER_PDF = DOWNLOADS / "52nd TOPIK II Answer Sheets.pdf"
LISTENING_AUDIO = DOWNLOADS / "52nd-TOPIK-II-Listening-Audio-File-64k-mono.mp3"

OUTPUT_SQL = ROOT / "backend" / "sql" / "seeds" / "mock_tests" / "topik_ii" / "topik_ii_52.sql"
ASSETS_ROOT = ROOT / "topik_ii_52_assets"
UPLOAD_ROOT = ASSETS_ROOT / "upload" / "topik-ii-52"
BACKEND_MEDIA_ROOT = ROOT / "backend" / "public" / "topik-ii-52"

STORAGE_BASE_URL = "https://ywtxdfwntobzegrlyplw.supabase.co/storage/v1/object/public/mock%20test%20files"
AUDIO_URL = f"{STORAGE_BASE_URL}/topik-ii-52/audio/52nd-TOPIK-II-Listening-Audio-File-64k-mono.mp3"
MOCK_TEST_ID_SQL = (
    "(SELECT id FROM mock_test_bank "
    "WHERE exam_type = 'TOPIK_II' AND test_number = 52 "
    "ORDER BY created_at DESC LIMIT 1)"
)

CIRCLED_TO_INDEX = {"①": 1, "②": 2, "③": 3, "④": 4, "❶": 1, "❷": 2, "❸": 3, "❹": 4, "➀": 1, "➁": 2, "➂": 3, "➃": 4}
LISTENING_QUESTION_PATTERN = re.compile(r"(?<!\d)(\d{1,2})\.(?=(?:\s|[가-힣A-Z\(]))")
READING_QUESTION_PATTERN = re.compile(r"(?<!\d)(\d{1,2})\.(?=(?:\s|[가-힣A-Z\(]))")

LISTENING_PAGE_RANGES = range(1, 27)
READING_PAGE_RANGES = range(24, 45)

LISTENING_IMAGE_URLS: dict[int, str] = {}

LISTENING_OPTION_IMAGE_URLS = {
    1: [f"{STORAGE_BASE_URL}/topik-ii-52/listening/q1_option_{index}.png" for index in range(1, 5)],
    2: [f"{STORAGE_BASE_URL}/topik-ii-52/listening/q2_option_{index}.png" for index in range(1, 5)],
    3: [f"{STORAGE_BASE_URL}/topik-ii-52/listening/q3_option_{index}.png" for index in range(1, 5)],
}

READING_IMAGE_URLS = {
    55: f"{STORAGE_BASE_URL}/topik-ii-52/reading/q55.png",
    56: f"{STORAGE_BASE_URL}/topik-ii-52/reading/q56.png",
    57: f"{STORAGE_BASE_URL}/topik-ii-52/reading/q57.png",
    58: f"{STORAGE_BASE_URL}/topik-ii-52/reading/q58.png",
    59: f"{STORAGE_BASE_URL}/topik-ii-52/reading/q59.png",
    60: f"{STORAGE_BASE_URL}/topik-ii-52/reading/q60.png",
}
...
LISTENING_IMAGE_ASSETS = {
    1: {"page": 1, "block_groups": [[0, 1], [2, 3], [4, 5], [6, 7]]},
    2: {"page": 2, "block_groups": [[0, 1], [2, 3], [4, 5], [6, 7]]},
    3: {"page": 3, "block_groups": [[0, 1], [2, 3], [4, 5], [6, 7]]},
}

READING_IMAGE_ASSETS = {
    55: {"page": 25, "block_indices": [0, 1, 2]},
    56: {"page": 25, "block_indices": [3, 4, 5]},
    57: {"page": 25, "block_indices": [6, 7, 8]},
    58: {"page": 25, "block_indices": [9, 10, 11]},
    59: {"page": 26, "block_indices": [0, 1, 2, 3, 4]},
    60: {"page": 26, "block_indices": [5, 6, 7, 8, 9, 10]},
}
...
def parse_reading_answers() -> dict[int, tuple[int, int]]:
    order = [value for pair in zip(range(1, 26), range(26, 51)) for value in pair]
    answers = parse_answer_page(1, order)
    if len(answers) != 50:
...
def build_sql(questions: list[dict[str, object]]) -> str:
    lines = [
        ...
        ");",
        "",
        f"UPDATE mock_test_bank SET title = 'TOPIK II 52-р шалгалт', description = '52-р албан ёсны TOPIK II шалгалт', total_questions = 100, duration = 130, listening_questions = 50, reading_questions = 50, updated_at = NOW() WHERE id = {MOCK_TEST_ID_SQL};",
        "",
...
def build_assets() -> None:
    ...
    for question_number, meta in LISTENING_IMAGE_ASSETS.items():
        ...
        for index, group in enumerate(meta["block_groups"], start=1):
