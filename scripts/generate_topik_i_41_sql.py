...
PAPERS_PDF = DOWNLOADS / "41st TOPIK I - Listening & Reading Paper.pdf"
LISTENING_PDF = DOWNLOADS / "41st TOPIK I - Listening Text.pdf"
ANSWER_PDF = DOWNLOADS / "41st TOPIK I - Answer Sheet.pdf"
LISTENING_AUDIO = DOWNLOADS / "41st-TOPIK-I-Listening-Audio-File.mp3"

OUTPUT_SQL = ROOT / "backend" / "sql" / "seeds" / "mock_tests" / "topik_i" / "topik_i_41.sql"
ASSETS_ROOT = ROOT / "topik_i_41_assets"
UPLOAD_ROOT = ASSETS_ROOT / "upload" / "topik-i-41"
BACKEND_MEDIA_ROOT = ROOT / "backend" / "public" / "topik-i-41"

STORAGE_BASE_URL = "https://ywtxdfwntobzegrlyplw.supabase.co/storage/v1/object/public/mock%20test%20files"
AUDIO_FILENAME = "41st-TOPIK-I-Listening-Audio-File.mp3"
AUDIO_URL = f"{STORAGE_BASE_URL}/topik-i-41/audio/{AUDIO_FILENAME}"
MOCK_TEST_ID_SQL = (
    "(SELECT id FROM mock_test_bank "
    "WHERE exam_type = 'TOPIK_I' AND test_number = 41 "
...
LISTENING_OPTION_IMAGE_URLS = {
    15: [f"{STORAGE_BASE_URL}/topik-i-41/listening/q15_option_{index}.png" for index in range(1, 5)],
    16: [f"{STORAGE_BASE_URL}/topik-i-41/listening/q16_option_{index}.png" for index in range(1, 5)],
}

READING_IMAGE_URLS = {
    40: f"{STORAGE_BASE_URL}/topik-i-41/reading/q40.png",
    41: f"{STORAGE_BASE_URL}/topik-i-41/reading/q41.png",
    42: f"{STORAGE_BASE_URL}/topik-i-41/reading/q42.png",
    63: f"{STORAGE_BASE_URL}/topik-i-41/reading/q63_64.png",
    64: f"{STORAGE_BASE_URL}/topik-i-41/reading/q63_64.png",
}
...
def clean_reading_text(text: str) -> str:
    ...
    for raw_line in text.splitlines():
        ...
        if "한국어능력시험I" in line:
            continue
        if "TOPIKⅠ" in line and "읽기" in line:
            continue
        if "TO PIKⅠ" in line and "읽기" in line:
            continue
        if re.fullmatch(r"\d+", line):
            continue
        lines.append(line)
...
def build_question_blocks(text: str, start: int, end: int) -> dict[int, str]:
    if start == 1 and end == 30:
        pattern = re.compile(r"(?<!\d)(\d{1,2})\.(?=\(\d+점\)|[가-힣])")
...
def build_sql(questions: list[dict[str, object]]) -> str:
    lines = [
        "-- Generated by scripts/generate_topik_i_41_sql.py",
        "BEGIN;",
        "",
        ...
        ")",
        "SELECT",
        "  'TOPIK I 41-р шалгалт',",
        "  'TOPIK_I',",
        "  41,",
        "  '41-р албан ёсны TOPIK I шалгалт',",
        "  70,",
        "  100,",
        "  30,",
        "  40",
        "WHERE NOT EXISTS (",
        "  SELECT 1 FROM mock_test_bank WHERE exam_type = 'TOPIK_I' AND test_number = 41",
        ");",
        "",
        f"UPDATE mock_test_bank SET title = 'TOPIK I 41-р шалгалт', description = '41-р албан ёсны TOPIK I шалгалт', total_questions = 70, duration = 100, listening_questions = 30, reading_questions = 40, updated_at = NOW() WHERE id = {MOCK_TEST_ID_SQL};",
        "",
